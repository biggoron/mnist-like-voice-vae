...
from utils.utils import variable_on_cpu
...
def BiRNN(conf_path, batch_x, seq_length, n_input, n_context):
    ...
    n_cell_dim1 = parser.getint('birnn', 'n_cell_dim1')
    n_cell_dim2 = parser.getint('birnn', 'n_cell_dim2')
    n_cell_dim3 = parser.getint('birnn', 'n_cell_dim3')
    ...
    with tf.name_scope('lstm'):
        ...
        lstm_fw_cell1 = tf.contrib.rnn.BasicLSTMCell(
          n_cell_dim1,
          forget_bias=1.0,
          state_is_tuple=True
        )
        lstm_fw_cell1 = tf.contrib.rnn.DropoutWrapper(
          lstm_fw_cell1,
        ...
        lstm_bw_cell1 = tf.contrib.rnn.BasicLSTMCell(
          n_cell_dim1,
          forget_bias=1.0,
          state_is_tuple=True
        )
        lstm_bw_cell1 = tf.contrib.rnn.DropoutWrapper(
          lstm_bw_cell1,
        ...
        # Forward direction cell:
        lstm_fw_cell2 = tf.contrib.rnn.BasicLSTMCell(
          n_cell_dim2,
          forget_bias=1.0,
          state_is_tuple=True
        )
        ...
        # Backward direction cell:
        lstm_bw_cell2 = tf.contrib.rnn.BasicLSTMCell(
          n_cell_dim2,
          forget_bias=1.0,
          state_is_tuple=True
        )
        ...
        # Forward direction cell:
        lstm_fw_cell3 = tf.contrib.rnn.BasicLSTMCell(
          n_cell_dim3,
          forget_bias=1.0,
          state_is_tuple=True
        )
        ...
        # Backward direction cell:
        lstm_bw_cell3 = tf.contrib.rnn.BasicLSTMCell(
          n_cell_dim3,
          forget_bias=1.0,
          state_is_tuple=True
        )
        ...
        outputs1, output_states = tf.nn.bidirectional_dynamic_rnn(
          cell_fw=lstm_fw_cell1,
        ...
        outputs2, output_states = tf.nn.bidirectional_dynamic_rnn(
          cell_fw=lstm_fw_cell2,
        ...
        outputs3, output_states = tf.nn.bidirectional_dynamic_rnn(
          cell_fw=lstm_fw_cell3,
          cell_bw=lstm_bw_cell3,
          inputs=outputs2,
          dtype=tf.float32,
          time_major=True,
          sequence_length=seq_length,
        ...
        outputs = tf.reshape(outputs, [-1, 2 * (n_cell_dim1 + n_cell_dim2 + n_cell_dim3)])
    ...
    with tf.name_scope('fc5'):
        ...
        h5 = variable_on_cpu(
          'h5',
          [(2 * (n_cell_dim1 + n_cell_dim2 + n_cell_dim3)), n_hidden_5],
          tf.contrib.layers.xavier_initializer()
        )
    ...
    with tf.name_scope('fc7'):
        ...
        layer_7 = tf.minimum(tf.nn.relu(tf.add(tf.matmul(layer_5, h7), b7)), relu_clip)
        layer_7 = tf.nn.dropout(layer_7, (1.0 - dropout[5]))

        tf.summary.histogram("weights", h7)
        tf.summary.histogram("biases", b7)
        tf.summary.histogram("activations", layer_7)
    ...
    with tf.name_scope('fc6'):
        ...
        layer_6 = tf.add(tf.matmul(layer_7, h6), b6)
