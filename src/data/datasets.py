...
class DataSet:
    ...
    def __init__(self, txt_files, thread_count, batch_size, numcep, numcontext, w_wavelets):
        self._coord = None
        self._numcep = numcep
        ...
        self._start_idx = 0
        if w_wavelets == 0:
          self._w_wavelets = False
        else:
          self._w_wavelets = True

    @classmethod
    def from_directory(cls, dirpath, thread_count, batch_size, numcep, numcontext, start_idx=0, limit=0, sort=None, w_wavelets = True):
        ...
        return cls(txt_files, thread_count, batch_size, numcep, numcontext, w_wavelets)
    ...
    def next_batch(self, batch_size=None):
        ...
        idx_list = [x % len(self._txt_files) for x in list(range(self._start_idx, end_idx))]
        txt_files = [self._txt_files[i] for i in idx_list]
        wav_files = [x.replace('.txt', '.wav') for x in txt_files]
        (source, target) = get_audio_and_transcript(txt_files,
                                                          wav_files,
                                                          self._numcep,
                                                          self._numcontext,
                                                          self._w_wavelets)
        self._start_idx = (self._start_idx + batch_size) % len(self._txt_files)
...
def _get_data_set_dict(conf_path, sets):
    ...
    if 'train' in sets:
        ...
        d['w_wavelets'] = parser.getint(config_header, 'with_wavelets')
        data_dict['train'] = d
